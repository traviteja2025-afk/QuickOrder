                        ...
                        {products.map(product => (
                            <div key={product.id} className="flex items-center space-x-4 p-4 border rounded-lg bg-slate-50">
                                ...
                                <div className="flex flex-col sm:flex-row space-y-2 gap-2">
                                    <button onClick={() => { setEditingProduct(product); setProductToDelete(null); window.scrollTo({top:0, behavior:'smooth'}); }} className="px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded hover:bg-blue-200">Edit</button>
                                    <button onClick={() => { setProductToDelete(product); setEditingProduct(null); window.scrollTo({top:0, behavior:'smooth'}); }} className="px-3 py-1 text-sm bg-red-100 text-red-700 rounded hover:bg-red-200">Delete</button>
                                </div>
                            </div>
                        ))}
                    </div>
